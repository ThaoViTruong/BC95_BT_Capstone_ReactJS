...
const getShowtimeEntries = (movies, dayOffset) => {
  ...
  return HALLS.flatMap((hall, hallIndex) => {
    ...
    return Array.from({ length: entryCount }, (_, entryIndex) => {
      const movieIndex =
        (hallIndex * 2 + entryIndex + dayOffset) %
        Math.max(sourceMovies.length, 1);
      ...
      const statusKey = ["active", "soldOut", "ended"][
        (hallIndex + entryIndex + dayOffset) % 3
      ];
      ...
      return {
        ...
        statusKey,
        occupancy:
          statusKey === "soldOut" ? 100 : statusKey === "active" ? 82 : 21,
      };
    });
  });
};
  ...
  return (
    ...
    >
      {entry.poster ? (
        ...
        <div className="flex h-16 w-12 items-center justify-center rounded-xl bg-white/5 text-xs text-white/35">
          N/A
        </div>
      )}
      <div className="min-w-0 flex-1">
        ...
        <span
          className={`mt-3 inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold tracking-[0.2em] ${style.badge}`}
        >
          {style.label}
        </span>
      </div>
...
const ShowtimeManagementPage = () => {
  ...
  const filteredEntries = useMemo(() => {
    ...
    return allEntries.filter((entry) => {
      ...
      return [entry.title, hall?.name, hall?.format].some((value) =>
        String(value || "")
          .toLowerCase()
  ...
  const entriesByHall = useMemo(() => {
    return HALLS.reduce((result, hall) => {
      result[hall.id] = filteredEntries.filter(
        (entry) => entry.hallId === hall.id,
      );
      return result;
    }, {});
  }, [filteredEntries]);
  ...
  const activeHallCount = HALLS.filter(
    (hall) => (entriesByHall[hall.id] || []).length > 0,
  ).length;
  const occupancyRate = filteredEntries.length
    ? Math.round(
        filteredEntries.reduce((total, entry) => total + entry.occupancy, 0) /
    ...
    <div className="space-y-8 font-sans text-white">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        ...
        <StatCard
          ...
          value={`${activeHallCount}/${HALLS.length}`}
          subValue="Số phòng có lịch chiếu trong ngày"
        />
        <StatCard
          ...
          label="Tỷ lệ lấp đầy"
          value={`${occupancyRate}%`}
          subValue="Ước tính theo trạng thái suất chiếu"
        />
      </div>
